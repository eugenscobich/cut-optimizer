...
import { OpenCascadeInstance } from './opencascade-loader.service';
import {
  AmbientLight,
  DirectionalLight,
  PerspectiveCamera,
...
export class OpenCascadeService {
  ...
  makeBottle(openCascade: OpenCascadeInstance, myWidth: number, myHeight: number, myThickness: number) {
    const aBoxBuilder = new openCascade['BRepPrimAPI_MakeBox'](myWidth, myHeight, myThickness);
    const aShape = aBoxBuilder.Solid();
    return aShape;
  }


  async addShapeToScene(openCascade: OpenCascadeInstance, shape, scene: Scene) {
    const facelist = await tessellate(shape);
    const [locVertexcoord, locNormalcoord, locTriIndices] = await joinPrimitives(facelist);
    const tot_triangle_count = facelist.reduce((a, b) => a + b.number_of_triangles, 0);
    const [vertices, faces] = await generateGeometry(tot_triangle_count, locVertexcoord, locNormalcoord, locTriIndices);
